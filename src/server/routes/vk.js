...
module.exports = [
	...
	{
  method: 'GET',
  path: '/vk/refreshAlbums',
  handler: function (request, reply) {
    global.App.vk.sendRequest('video.getAlbums', {
      data: {
  	    owner_id: global.App.vk.isAuthorized(),
				offset: 0,
				need_system: 1,
  	    count: 100,
  	    extended: 1
			...
			success: function(err, response) {
				if(err) {
					reply(global.App.formatErrorResponse('ApplicationError', 'Something goes wrong', 'See data for details.', err, response.request));
				}
				let raw = JSON.parse(response.body);
				let myReply = {
					responseText: 'callback',
					albums: {
						total: raw.response.shift(),
						items: raw.response
					}
				};
				reply(myReply);
